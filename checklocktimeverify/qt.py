...
class Plugin(BasePlugin):
    ...
    def __init__(self, parent, config, name):
        ...
        self.logger.info("=" * 80)
        self.logger.info(f"[CLTV] CLTV Plugin {version_str} Initializing...")
        self.logger.info(f"[CLTV]   - Plugin name: {name}")
        self.logger.info(f"[CLTV]   - Git commit: {commit_hash}")
        self.logger.info(f"[CLTV]   - Commit date: {commit_date}")
        self.logger.info("=" * 80)
        self.logger.info(f"[CLTV] Plugin {version_str} initialized")
        self.logger.info(f"[CLTV]   Git: {commit_hash} @ {commit_date}")
    ...
    def get_lock_status(self, wallet, locktime: int) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error(f"[CLTV] [HELPER] get_lock_status error (locktime={locktime}): {e}")
            return {
                'locked': True,
                'blocks_remaining': 0,
                'current_height': 0,
    ...
    def get_default_locktime(self, wallet, *, offset_blocks: int = 3) -> int:
        ...
        try:
            ...
            self.logger.warning(f"[CLTV] [HELPER] get_default_locktime failed: {e}")
            return max(1, int(offset_blocks))

    @hook
    def load_wallet(self, wallet, window):
        """Called when a wallet is loaded - add CLTV tab + start monitoring"""
        self.logger.info(f"[CLTV] load_wallet called for window: {window}")
    ...
    def close_wallet(self, wallet):
        ...
        self.logger.info(f"[CLTV] close_wallet called")
    ...
    def _refresh_cltv_list(self, wallet=None):
        if not (hasattr(self, 'cltv_list') and self.cltv_list):
            return
        wallets = [wallet] if wallet is not None else list(self.wallet_windows.keys())
        for target_wallet in wallets:
            self.cltv_list.update_rows.emit(target_wallet)
    ...
    def on_history(self, wallet, *args):
        ...
        self.logger.debug(f"[CLTV] [HOOK] on_history: emitting update_rows for wallet {id(wallet)}")
        self._refresh_cltv_list(wallet)
    ...
    def blockchain_updated(self, *args):
        ...
        self._refresh_cltv_list()
        # Dialogs handle their own refresh via QtEventListener
    
    @hook
    def wallet_updated(self, wallet, *args):
        """Called when wallet state changes - refresh UI."""
        self.logger.debug(f"[CLTV] [HOOK] wallet_updated: emitting update_rows for wallet {id(wallet)}")
        self._refresh_cltv_list(wallet)
        # Dialogs handle their own refresh via QtEventListener
    ...
    def _init_address_monitor(self, wallet):
        ...
        try:
            ...
            self.logger.error(f"[CLTV] [WALLET] Error registering addresses: {e}")
            import traceback
            traceback.print_exc()
        ...
        try:
            ...
            for addr_data in address_list:
                ...
                if wallet.is_mine(addr):
                    self.logger.debug(f"[CLTV] [WALLET] Address already tracked: {addr[:20]}...")
                    continue
                ...
                if hasattr(wallet, 'import_address'):
                    try:
                        ...
                        self.logger.info(f"[CLTV] [WALLET] Registered via import_address: {addr[:20]}...")
                        registered_count += 1
                        registered = True
                    except AttributeError:
                        self.logger.debug(f"[CLTV] [WALLET] import_address not available for: {addr[:20]}...")
                    except Exception as e:
                        self.logger.debug(f"[CLTV] [WALLET] import_address failed for {addr[:20]}...: {e}")

                if not registered and hasattr(wallet, 'adb') and hasattr(wallet.adb, 'add_address'):
                    try:
                        wallet.adb.add_address(addr)
                        self.logger.info(f"[CLTV] [WALLET] Registered via ADB.add_address: {addr[:20]}...")
                        registered_count += 1
                        registered = True
                    except Exception as e:
                        self.logger.warning(f"[CLTV] [WALLET] ADB.add_address failed for {addr[:20]}...: {e}")

                if not registered:
                    self.logger.warning(f"[CLTV] [WALLET] No registration method worked for: {addr[:20]}...")

            if registered_count > 0:
                self._trigger_wallet_sync(wallet)
            self.logger.info(f"[CLTV] [WALLET] Registered {registered_count} CLTV addresses with Electrum")
    ...
    def _trigger_wallet_sync(self, wallet):
        ...
        try:
            ...
            if hasattr(wallet, 'synchronize'):
                try:
                    wallet.synchronize()
                    self.logger.debug(f"[CLTV] [SYNC] Triggered wallet sync")
                    return
                except Exception as e:
                    self.logger.debug(f"[CLTV] [SYNC] Wallet synchronize failed: {e}")
            ...
            if hasattr(wallet, 'adb') and wallet.adb:
                ...
                if hasattr(adb, 'synchronizer') and adb.synchronizer:
                    if hasattr(adb.synchronizer, 'synchronize'):
                        ...
                        self.logger.info(f"[CLTV] [SYNC] Triggered ADB synchronizer sync")
                        
        except Exception as e:
            self.logger.debug(f"[CLTV] [SYNC] Could not trigger sync: {e}")
    ...
    def add_cltv_tab(self, window):
        ...
        try:
                    ...
                    if tab_text == _("Contracts"):
                        ...
                        if widget and hasattr(widget, 'list_widget'):
                            ...
                            if isinstance(list_widget, CLTVList):
                                ...
                                self.logger.info(f"[CLTV] [TAB] Found valid CLTV tab at index {i}")
                except Exception:
                    continue
            
            # If we found a valid tab, reuse it
            if valid_tab_index is not None and valid_list_widget is not None:
                self.logger.info(f"[CLTV] [TAB] Reusing existing valid CLTV tab")
                ...
                for i in reversed(contracts_tab_indices):
                    if i != valid_tab_index:
                        self.logger.info(f"[CLTV] [TAB] Removing duplicate/invalid Contracts tab at index {i}")
                        try:
                            widget = window.tabs.widget(i)
                            ...
                            self.logger.info(f"[CLTV] [TAB] Error removing tab at index {i}: {e}")
                # Refresh data
            ...
            if contracts_tab_indices:
                self.logger.info(f"[CLTV] [TAB] Found {len(contracts_tab_indices)} invalid Contracts tab(s) - removing")
                for i in reversed(contracts_tab_indices):
                    try:
                        widget = window.tabs.widget(i)
                        window.tabs.removeTab(i)
                        ...
                        self.logger.info(f"[CLTV] [TAB] Error removing invalid tab at index {i}: {e}")
            
            # Create new tab
            self.logger.info(f"[CLTV] [TAB] Creating new CLTV tab...")
            ...
            self.logger.info(f"[CLTV] [TAB] CLTV tab added successfully")
            
        except Exception as e:
            self.logger.error(f"[CLTV] [TAB] [ERROR] Failed to add CLTV tab: {e}")
            import traceback
            self.logger.info(f"[CLTV] [TAB] Traceback:\n{traceback.format_exc()}")
    ...
    def settings_widget(self, window):
        ...
        def on_debug_toggle(state):
            ...
            self.logger.info(f"[CLTV] Note: Debug logging is controlled via Electrum's standard logging configuration")
            self.logger.info(f"[CLTV] cltv_debug set to {val}")
        ...
        def on_offset_changed(val):
            try:
                ...
                self.logger.info(f"[CLTV] cltv_lock_offset set to {val}")
            except Exception:
                pass
        ...
        def on_branch_edit():
            ...
            try:
                ...
                self.logger.info(f"[CLTV] cltv_derivation_branch set to {val}")
            except Exception:
                pass
    ...
    def save_timelock_data(self, address: str, wallet, **data) -> None:
        ...
        self.logger.info(f"[CLTV] [STORAGE] Saving {data.get('script_type', 'unknown')} address {address[:20]}...")
        self.logger.debug(f"[CLTV] [STORAGE] DEBUG: Incoming data keys: {list(data.keys())}")

        if not hasattr(wallet, 'db'):
            raise RuntimeError("Wallet has no database attribute - cannot save address")

        cltv_data = self.get_storage(wallet)
        ...
        try:
            ...
            required_params = contract_obj.get_required_param_names()
            self.logger.info(f"[CLTV] [STORAGE] Contract requires: {required_params}")

            params = {}
            if 'params' in data and isinstance(data['params'], dict):
                self.logger.debug("[CLTV] [STORAGE] Found nested params, extracting...")
                nested_params = data['params']
                for param_name in required_params:
                    if param_name in nested_params:
                        params[param_name] = nested_params[param_name]
                        self.logger.info(f"[CLTV] [STORAGE] Found {param_name}: {str(nested_params[param_name])[:40]}...")
                    else:
                        self.logger.error(f"[CLTV] [STORAGE] Missing required param: {param_name}")
            else:
                for param_name in required_params:
                    if param_name in data:
                        params[param_name] = data[param_name]
                        self.logger.info(f"[CLTV] [STORAGE] Found {param_name}: {str(data[param_name])[:40]}...")
                    else:
                        self.logger.error(f"[CLTV] [STORAGE] Missing required param: {param_name}")
            ...
            self.logger.info("[CLTV] [STORAGE] All required parameters present")
        ...
        if 'locktime' in params:
            ...
            if locktime is None:
                self.logger.warning("[CLTV] [STORAGE] [WARNING] locktime is None, defaulting to 0")
                params['locktime'] = 0
            elif not isinstance(locktime, int):
                self.logger.warning(f"[CLTV] [STORAGE] [WARNING] locktime is {type(locktime)}, converting to int")
                params['locktime'] = int(locktime)

        param_summary = {
            k: (v if not isinstance(v, str) or len(v) < 20 else f"{v[:20]}...")
            for k, v in params.items()
        }
        self.logger.info(f"[CLTV] [STORAGE] Extracted params: {param_summary}")
        ...
        try:
            ...
            self.logger.warning(f"[CLTV] [STORAGE] Failed to regenerate for cache: {e}")

        self.update_address_cache(wallet, address, cache_entry)
        self._register_addresses_with_wallet(wallet, address=address)

        action = "Updated" if is_update else "Saved"
        self.logger.info(f"[CLTV] [STORAGE] {action} {script_type} ({len(cltv_data['addresses'])} total)")
    
    def load_all_addresses(self, wallet, use_cache: bool = True) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if storage_version != '12.0.0':
                ...
                self.logger.info(f"[CLTV] [STORAGE] Skipping addresses (storage version {version_str}, expected 12.0.0)")
                return []
            ...
            for addr, data in list(cltv_data['addresses'].items()):
                ...
                if not script_type or not params:
                    skipped_count += 1
                    self.logger.info(f"[CLTV] [STORAGE] Skipping {addr[:20]}... (missing script_type or params)")
                    continue
                ...
                try:
                    ...
                    self.logger.error(f"[CLTV] [STORAGE] CRITICAL: Failed to regenerate address {addr[:20]}...")
                    self.logger.error(f"[CLTV] [STORAGE] Script Type: {script_type}")
                    self.logger.error(f"[CLTV] [STORAGE] Params: {params}")
                    self.logger.error(f"[CLTV] [STORAGE] Error: {regen_err}")
                    self.logger.error(f"[CLTV] [STORAGE] Traceback:\n{tb_str}")
                    self.logger.error("[CLTV] [STORAGE] This address cannot be loaded and will be skipped.")

            if skipped_count > 0:
                self.logger.info(f"[CLTV] [STORAGE] Skipped {skipped_count} addresses (missing required fields)")
            if failed_count > 0:
                self.logger.error(f"[CLTV] [STORAGE] FAILED to load {failed_count} addresses (regeneration errors - see logs above)")
            ...
            try:
                ...
                self.logger.info(f"[CLTV] [WALLET] Could not register addresses on load: {e}")

            self.logger.info(f"[CLTV] [STORAGE] Loaded {len(addresses)} addresses (with regeneration)")
            type_counts = {}
            for addr in addresses:
                st = addr.get('script_type', 'unknown')
                type_counts[st] = type_counts.get(st, 0) + 1
            if type_counts:
                self.logger.info(f"[CLTV] [STORAGE] Breakdown: {dict(sorted(type_counts.items()))}")

            return addresses

        except Exception as e:
            self.logger.info(f"[CLTV] [STORAGE] ERROR loading: {e}")
            return []
    ...
    def update_address_cache(self, wallet, address: str, address_data: Dict[str, Any]):
        ...
        if 'script_hex' not in address_data or not address_data.get('script_hex'):
            try:
                ...
                if isinstance(script_type, str) and isinstance(params, dict) and script_type and params:
                    from .cltv_lib.address_regenerator import regenerate_address_data
                    regenerated = regenerate_address_data(script_type, params)
                    address_data.update(regenerated)
            except Exception as e:
                self.logger.warning(f"[CLTV] [CACHE] Failed to regenerate {address[:20]}: {e}")
    ...
    def delete_address(self, address: str, wallet) -> bool:
        ...
        try:
            ...
            if address in cltv_data['addresses']:
                ...
                wallet.db.write()
                self.invalidate_address_cache(wallet)
                self.logger.info(f"[CLTV] [CLEANUP] Deleted address: {address[:30]}...")
                return True

            return False

        except Exception as e:
            self.logger.info(f"[CLTV] [CLEANUP] ERROR deleting address: {e}")
            return False
    ...
    def set_address_label(self, wallet, address: str, label: str) -> bool:
        ...
        try:
            ...
            wallet.db.write()
            self.invalidate_address_cache(wallet)
            self.logger.debug(f"[CLTV] [LABEL] Set label for {address[:20]}...: {label}")
            return True

        except Exception as e:
            self.logger.info(f"[CLTV] [LABEL] ERROR setting label: {e}")
            return False
 ...
 WARNING: This deletes all addresses, including valid ones!
        ...
        """
        self.logger.warning(f"[CLTV] [CLEANUP] WARNING: Deleting ALL CLTV addresses!")
        ...
        try:
            ...
            self.logger.info(f"[CLTV] [CLEANUP] Deleted ALL {count} CLTV addresses")
            return {'deleted': count, 'addresses': all_addrs}

        except Exception as e:
            self.logger.info(f"[CLTV] [CLEANUP] ERROR: {e}")
